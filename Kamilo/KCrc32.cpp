...
uint32_t KCrc32::fromByte(uint8_t data, uint32_t crc) {
	return (crc >> 8) ^ g_Crc32Table[(crc ^ data) & 0xFF];
}
uint32_t KCrc32::fromData(const void *data, int size) {
	// CRC32 の計算を行う
...
uint32_t KCrc32::fromString(const char *str) {
	uint32_t crc = INIT;
	int i = 0;
	while (str[i]) {
		uint8_t b = ((const uint8_t*)str)[i];
		crc = fromByte(b, crc);
		i++;
	}
	return ~crc;
