...
void KImGui::Image(KTEXID tex, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col) {
	...
	p.u1 = uv1.x;
	p.v1 = uv1.x;
	p.color = KColor(tint_col.x, tint_col.y, tint_col.z, tint_col.w);
...
void KImGui::StyleKK() {
	...
	style.FramePadding  = ImVec2(4, 0); // ImGuiStyleVar_FramePadding
	style.ItemSpacing   = ImVec2(2, 1); // ImGuiStyleVar_ItemSpacing
...
#pragma region kanji range
// 文字コード範囲。非ascii 文字は二個ペアで入れることに注意。末尾は 0 で終わる
static const ImWchar g_GlyphRangesJapanese[] = {
...
#pragma endregion // kanji range

// Dear ImGui で日本語を表示するために必要な漢字テーブルを得る。
//
// Dear ImGui のデフォルトの状態だと2010以前の常用漢字しか正しく表示できない。
// 2010年に追加された常用漢字およびそれ以外の頻出漢字を Dear ImGui で表示できるようにするため、
// それらの漢字を含めたテーブルを作成し、それを ImGuiIO::Fonts の AddFontFromMemoryTTF で指定する
// @note 常用漢字以外の漢字で日常的に使う文字は意外と多いので注意。例えば「杖」や「牢」は常用漢字ではない。
//
// @code
...
/// この関数を呼んだ後でないと他の大部分の Dear ImGui 関数は
/// 使う事ができない（コンテキスト未設定エラーになる）
///
...
bool KImGui::InitWithDX9(void *hWnd, void *d3ddev9) {
	...
	// バインド済みでないことを確認
	if (g_d3d9_dev) return false;
	if (g_d3d9_block) return false;
...
//	ImGui::GetIO().ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard;  // Enable Keyboard Controls
	...
	// ImGui と Video を関連付ける
	g_d3d9_dev = (IDirect3DDevice9*)d3ddev9;
	g_d3d9_dev->AddRef();
	...
	{
		// 日本語フォントを追加 (Meiryo UI)
		// ※フォントのファイル名はアスキー文字だけが使われていると信じて、ユニコード対応しない
		char fontfile_u8[256] = {0};
...
/// マウスやキーボードなどのイベントを ImGui に処理させるために呼ぶ
/// @param hWnd WNDPROC の HWND に相当
/// @param msg  WNDPROC の UINT に相当
...
long KImGui::WndProc(void *hWnd, uint32_t msg, uintptr_t wp, uintptr_t lp) {
	...
	// 正しく処理しないと ImGui のテキストエディタにカーソルを重ねてもカーソル形状が IBeam にならない
	return ImGui_ImplWin32_WndProcHandler((HWND)hWnd, (UINT)msg, (WPARAM)wp, (LPARAM)lp);
}
